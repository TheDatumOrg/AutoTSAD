...
def run_iforest_dev(data, periodicity, n_estimators, n_jobs=1):
    ...
    if slidingWindow == 1:
        clf = IForest(n_estimators=n_estimators, n_jobs=n_jobs)
        clf.fit(data)
...
def run_lof_dev(data, periodicity, n_neighbors, n_jobs=1):
    slidingWindow = find_length_rank(data, rank=periodicity)
    X = Window(window = slidingWindow).convert(data).to_numpy()
    clf = LOF(n_neighbors=n_neighbors, n_jobs=n_jobs)
    clf.fit(X)
    score = clf.decision_scores_
    score = MinMaxScaler(feature_range=(0,1)).fit_transform(score.reshape(-1,1)).ravel()
    score = np.array([score[0]]*math.ceil((slidingWindow-1)/2) + list(score) + [score[-1]]*((slidingWindow-1)//2))
...
def run_matrix_profile_dev(data, periodicity, cross_correlation, n_jobs=1):
    slidingWindow = find_length_rank(data, rank=periodicity)
    clf = MatrixProfile(window = slidingWindow, cross_correlation=cross_correlation, n_jobs=n_jobs)
    clf.fit(data)
    score = clf.decision_scores_
    score = MinMaxScaler(feature_range=(0,1)).fit_transform(score.reshape(-1,1)).ravel()
    score = np.array([score[0]]*math.ceil((slidingWindow-1)/2) + list(score) + [score[-1]]*((slidingWindow-1)//2))
...
def run_pca_dev(data, periodicity, n_components, n_jobs=1):
    slidingWindow = find_length_rank(data, rank=periodicity)
    X = Window(window = slidingWindow).convert(data).to_numpy()
...
def run_hbos_dev(data, periodicity, n_bins, n_jobs=1):
    slidingWindow = find_length_rank(data, rank=periodicity)
    X = Window(window = slidingWindow).convert(data).to_numpy()
...
def run_ocsvm_dev(data, periodicity, kernel, n_jobs=1):
    slidingWindow = find_length_rank(data, rank=periodicity)
...
def run_ae_dev(data, periodicity, hidden_neurons, output_activation, norm, n_jobs=1):
    slidingWindow = find_length_rank(data, rank=periodicity)
