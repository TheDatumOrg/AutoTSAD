...
from TSB_UAD.models.norma_dev import NORMA
...
def run_iforest_unif(data, unif, periodicity, n_estimators):
    slidingWindow = find_length_rank(data, rank=periodicity)
    if slidingWindow == 1:
        clf = IForest(n_estimators=n_estimators)
        clf.fit(data)
        # score = clf.decision_scores_
        score = -clf.detector_.score_samples(unif.reshape(-1, 1))
        ...
        clf.fit(X)
        # score = clf.decision_scores_
        score = -clf.detector_.score_samples(Window(window = slidingWindow).convert(unif).to_numpy())
        score = MinMaxScaler(feature_range=(0,1)).fit_transform(score.reshape(-1,1)).ravel()
        score = np.array([score[0]]*math.ceil((slidingWindow-1)/2) + list(score) + [score[-1]]*((slidingWindow-1)//2))
    return score

def run_lof_unif(data, unif, periodicity, n_neighbors):
    slidingWindow = find_length_rank(data, rank=periodicity)
    X = Window(window = slidingWindow).convert(data).to_numpy()
    clf = LOF(n_neighbors=n_neighbors, n_jobs=-1)
    clf.fit(X)
    # score = clf.decision_scores_
    score = -clf.detector_._score_samples(Window(window = slidingWindow).convert(unif).to_numpy())
    score = MinMaxScaler(feature_range=(0,1)).fit_transform(score.reshape(-1,1)).ravel()
    score = np.array([score[0]]*math.ceil((slidingWindow-1)/2) + list(score) + [score[-1]]*((slidingWindow-1)//2))
    return score

def run_matrix_profile_unif(data, unif, periodicity, cross_correlation):
    slidingWindow = find_length_rank(data, rank=periodicity)
...
def run_pca_unif(data, unif, periodicity, n_components):
    slidingWindow = find_length_rank(data, rank=periodicity)
    X = Window(window = slidingWindow).convert(data).to_numpy()
    clf = PCA(n_components=n_components)
    clf.fit(X)
    # score = clf.decision_scores_
    score = clf.decision_function(Window(window = slidingWindow).convert(unif).to_numpy())
    score = MinMaxScaler(feature_range=(0,1)).fit_transform(score.reshape(-1,1)).ravel()
    score = np.array([score[0]]*math.ceil((slidingWindow-1)/2) + list(score) + [score[-1]]*((slidingWindow-1)//2))      
    return score

def run_norma_unif(data, unif, periodicity, clustering):
    slidingWindow = find_length_rank(data, rank=periodicity)
    ...
    score = MinMaxScaler(feature_range=(0,1)).fit_transform(score.reshape(-1,1)).ravel()
    score = np.array([score[0]]*math.ceil((slidingWindow-1)/2) + list(score) + [score[-1]]*((slidingWindow-1)//2))
    if len(score) > len(unif):
        start = len(score) - len(unif)
        score = score[start:]
    return score

def run_hbos_unif(data, unif, periodicity, n_bins):
    slidingWindow = find_length_rank(data, rank=periodicity)
    X = Window(window = slidingWindow).convert(data).to_numpy()
...
def run_poly_unif(data, unif, periodicity, power):
    slidingWindow = find_length_rank(data, rank=periodicity)
...
def run_ocsvm_unif(data, unif, periodicity, kernel):
    slidingWindow = find_length_rank(data, rank=periodicity)
    ...
    X_test = Window(window = slidingWindow).convert(unif).to_numpy()
...
def run_ae_unif(data, unif, periodicity, hidden_neurons, output_activation, norm):
    slidingWindow = find_length_rank(data, rank=periodicity)
...
def run_cnn_unif(data, unif, periodicity, num_channel, activation):
    slidingWindow = find_length_rank(data, rank=periodicity)
...
def run_lstm_unif(data, unif, periodicity, hidden_dim, activation):
    slidingWindow = find_length_rank(data, rank=periodicity)
