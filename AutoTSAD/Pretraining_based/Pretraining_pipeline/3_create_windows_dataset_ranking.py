...
def create_tmp_dataset(
    ...
    name = '{}_{}_ranking_value'.format(name, window_size)
    ...
    # print('x:{} metrics_data:{} window_size:{}'.format(x, metrics_data, window_size))
    print('metrics_data: ', metrics_data.columns)
    ...
    for ts, label, fname in tqdm(zip(ts_list, labels, fnames), total=len(ts_list), desc='Save dataset'):
        ...
        data = np.concatenate((label, ts), axis=1)
        # col_names = []
        # col_names += ['label_{}'.format(i) for i in range(label.shape[1])]      # Model: Ranking  --> detector_names[i-1]
        col_names = []
        col_names += detector_names
...
def split_and_compute_labels(x, metrics_data, window_size):
    ...
    # for ts, metric_label in tqdm(zip(x, metrics_data.rank(axis=1, ascending=False).values.tolist()), total=len(x), desc="Create dataset"):
    for ts, metric_label in tqdm(zip(x, metrics_data.values.tolist()), total=len(x), desc="Create dataset"):
        # print('metric_label: ', metric_label)
        # print('metrics_data.idxmax(axis=1):', metrics_data.idxmax(axis=1))
        # print('metric_ranking: ', metrics_data.rank(axis=1))
        ...
        for i in range(len(ts_split)):
            labels_ranking.append(metric_label)
        labels.append(np.array(labels_ranking))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--save_dir', type=str, help='path to save the dataset', default='data/')
    parser.add_argument('-p', '--path', type=str, help='path of the dataset to divide', default=TSB_data_path)
    parser.add_argument('-mp', '--metric_path', type=str, help='path to the metrics of the dataset given', default=TSB_metrics_path)
    parser.add_argument('-w', '--window_size', type=int, help='window size to segment the timeseries to', default=1024)
