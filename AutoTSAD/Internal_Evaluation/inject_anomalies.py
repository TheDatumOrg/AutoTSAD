...
def gen_synthetic_performance_list(data, label, Det_pool):
    ...
    for det in Det_pool:
        ...
        if det_name == 'IForest':
            score_list.append(run_iforest_dev(data, periodicity=int(det_hp[0]), n_estimators=int(det_hp[1])))
        elif det_name == 'LOF':
            score_list.append(run_lof_dev(data, periodicity=int(det_hp[0]), n_neighbors=int(det_hp[1])))
        elif det_name == 'MP':
            score_list.append(run_matrix_profile_dev(data, periodicity=int(det_hp[0]), cross_correlation=bool(det_hp[1])))
        elif det_name == 'PCA':
            det_hp[1]=None if det_hp[1] == 'None' else float(det_hp[1])
            score_list.append(run_pca_dev(data, periodicity=int(det_hp[0]), n_components=det_hp[1]))
        elif det_name == 'NORMA':
            score_list.append(run_norma_dev(data, periodicity=int(det_hp[0]), clustering=det_hp[1]))
        elif det_name == 'HBOS':
            score_list.append(run_hbos_dev(data, periodicity=int(det_hp[0]), n_bins=int(det_hp[1])))
        elif det_name == 'POLY':
            score_list.append(run_poly_dev(data, periodicity=int(det_hp[0]), power=int(det_hp[1])))
        elif det_name == 'OCSVM':
            score_list.append(run_ocsvm_dev(data, periodicity=int(det_hp[0]), kernel=det_hp[1]))
        elif det_name == 'AE':
            hidden_neurons_list = [[64, 32, 32, 64], [32, 16, 32]]
            score_list.append(run_ae_dev(data, periodicity=int(det_hp[0]), hidden_neurons=hidden_neurons_list[int(det_hp[1])], output_activation='relu', norm=det_hp[2]))
        elif det_name == 'CNN':
            num_channel_list = [[32, 32, 40], [32, 64, 64]]
            score_list.append(run_cnn_dev(data, periodicity=int(det_hp[0]), num_channel=num_channel_list[int(det_hp[1])], activation=det_hp[2]))
        elif det_name == 'LSTM':
            hidden_dim_list = [32, 64]
            score_list.append(run_lstm_dev(data, periodicity=int(det_hp[0]), hidden_dim=hidden_dim_list[int(det_hp[1])], activation=det_hp[2]))
    for score in score_list:
        synthetic_performance_list.append(metrics.average_precision_score(label, score))
