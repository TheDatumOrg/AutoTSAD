...
def get_excess_mass(data, det_scores, Det_Pool, row, alpha_min=0.9, alpha_max=0.999,
                       n_generated=100000, t_max = 0.9):
    ...
    for i, det in enumerate(Det_Pool):
        ...
        s_unif_clf = np.load('/data/liuqinghua/code/ts/TSAD-AutoML/AutoAD_Solution/candidate_pool/unif_clf/'+row['Dataset']+'/'+row['File_name']+'/'+det+'.npy')* -1
...
def get_mass_volume(data, det_scores, Det_Pool, row, alpha_min=0.9, alpha_max=0.999,
                       n_generated=100000, t_max = 0.9):
